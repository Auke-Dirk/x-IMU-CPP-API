...
// Constructors
  ...
  std::vector<T> eulerAngles() const {
    T phi = static_cast<T>(atan2(2 * (_y * _z - _w * _x), 2 * _w * _w - 1 + 2 * _z * _z));
    T theta = static_cast<T>(-atan((2.0 * (_x * _z + _w * _y)) /
             sqrt(1.0 - pow((2.0 * _x * _z + 2.0 * _w * _y), 2.0))));
    T psi = static_cast<T>((2 * (_x * _y - _w * _z), 2 * _w * _w - 1 + 2 * _x * _x));

    return std::vector<T>{ Math::rad2Deg(phi), Math::rad2Deg(theta), Math::rad2Deg(psi) };
  }
};
